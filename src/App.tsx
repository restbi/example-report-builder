...
function App() {
  ...
  // useEffect(() => {
  //   client.getMetadata(activeModel.connection).then((result) => {
  //     setActiveModel({
  //       ...activeModel,
  //       tables: result
  //     });
    ...
    <div className="flex h-screen w-screen flex-col">
      <div className="flex flex-row bg-white items-center pl-4 border-b-2" style={{height:'70px'}}>
        ...
        <div className='flex flex-row ml-auto'>
          <Select className='w-48 mr-2 my-2' value={activeModel.name} onChange={(e) => {
      ...
      <div className="flex flex-row h-full w-full">
        ...
        <TextField label="Search Columns" className='w-full border-2 border-blue-600 bg-gray-700 rounded-md text-white' type='text' value={searchString} onChange={
          ...
          <div className='flex flex-col mb-4' key={table.id}>
            ...
            {table.columns.filter((column: Column) => {
              ...
              <div className='flex flex-row items-center pr-2 py-2  hover:bg-gray-700' key={column.name}>
                ...
                <label className={' w-full ' + ((inferColumnType(column) == ColumnType.DIMENSION) ? 'text-blue-600' : 'text-green-600')} htmlFor={column.name}>{column.name}</label>
                  <button onClick={() => {
                    setFilters([...filters, {
        ...
        {activeModel.formulas.filter((formula: Formula) => {
          ...
          <div className='flex flex-row items-center pr-2 py-2  hover:bg-gray-700' key={formula.id}>
            ...
            id={formula.name} name={formula.name} value={formula.name}/>
        ...
        <div className='flex flex-col px-4 mt-4'>
          <div className='flex flex-row space-x-2'>
            ...
            <div className='flex flex-col w-48'>
              <TextField label="Limit" className='border-2 border-blue-600' type='number' onChange={
                (e) => setQuery({
                  ...query,
                  limit: parseInt(e.target.value)
              })} value={query.limit} placeholder='Enter limit'/>
          </div>
          </div>
          <div className='flex flex-row p-4 space-x-2'>
            {filters.map((filter) => (
              <div className='flex flex-row p-1 border-2 rounded-md mt-2' key={filter.column}>
              <Filter key={filter.column} column={
        ...
        <div
          className="w-full p-4 ag-theme-quartz" // applying the Data Grid theme
